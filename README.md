...
Appraiser isolation. The client assigns; the portal never suggests, ranks, rotates or recommends an appraiser, and it holds no assignment logic of any kind. An appraiser account is scoped to the orders assigned to it: the board, the order detail, the record, documents, file downloads, the outbox and feedback all filter on `assigned_to`, and unmatched client replies and the panel roster are not served to appraisers at all. The scoping lives in the queries in `src/worker.js` (`mineOnly` and `gateOrder`), not in the interface, so it holds for any client that reaches the API directly. Refusals are 404 rather than 403, so one appraiser is not told that another's order exists. This matters beyond good manners: Apprifi is operated by a practising appraiser who competes for lender work, and the separation is what keeps one appraiser's fees, turn times and client contacts out of a competitor's hands. `test/v2.js` covers it with two appraisers on the same lender.
